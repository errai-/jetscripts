...
void Compare(string file1, string file2, string file3, string file4, string file5, string file6)
{
	...
	for(unsigned int q=0; q != 6; ++q) //loop over files
	{
        ...
        for(size_t x=0; x != N; ++x)
        {
            ...
            for (int i = 0; i < fJets_; ++i) {
                ...
                if(fFlav[i] == 21) {
                    plots[q][0][2]->Fill(fConstituents[i],fWeight);
                    plots[q][0][1]->Fill(fPTD[i],fWeight);
                    plots[q][0][0]->Fill(fSigma2[i],fWeight);
                } else if(fFlav[i] == 0) {
                    plots[q][2][2]->Fill(fConstituents[i],fWeight);
                    plots[q][2][1]->Fill(fPTD[i],fWeight);
                    plots[q][2][0]->Fill(fSigma2[i],fWeight);
                } else {
                    plots[q][1][2]->Fill(fConstituents[i],fWeight);
                    plots[q][1][1]->Fill(fPTD[i],fWeight);
                    plots[q][1][0]->Fill(fSigma2[i],fWeight); 
                }
            }
        }
	}
