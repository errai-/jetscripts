...
void Append(TH2D* th, double eta, double phi, double pt) {
    for (int eta_ind = 0, Ne = th->GetNbinsX(); eta_ind < Ne; ++eta_ind) {
    for (int phi_ind = 0, Np = th->GetNbinsY(); phi_ind < Np; ++phi_ind) {
...
void EtaPhi::Loop()
{
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
        ...
        c1->SetLogz(1);
        TH2D* grid = new TH2D("","",50,-5,5,50,-TMath::Pi(),TMath::Pi());
        grid->SetMaximum(1000);
        grid->SetMinimum(0);
        ...
        for ( int j = 0; j < fJets; ++j ) {
            ...
            if ( fl == 7 || fl == 8 ) { // Final state
                Append( grid, t.Rapidity(), phif(t.Phi(),phi_sum), t.Et() );
            }
        }
        gStyle->SetOptStat(0);
        grid->SetXTitle("y");
