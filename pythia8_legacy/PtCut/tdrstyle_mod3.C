...
// Used by simulations of cut pT, drawPtCut.C
...
// For the Global title:
  ...
  // tdrStyle->SetTitleY(0.93); // Set the position of the title box
...
// For the axis titles:
  ...
  tdrStyle->SetTitleYOffset(1.25); // 1.25 => 1.15 if exponents
...
void pythiaFinal(double intLumi=-1, bool wide = false) {
  ...
  latex->DrawLatex(wide ? 0.98 : 0.96, 0.96, "#sqrt{s} = 14 TeV");
  if (intLumi > 0.) {
...
void cmsPrel(double intLumi=-1, bool wide = false) {
  ...
  if (intLumi > 0.) {
    ...
    latex->SetTextAlign(11); // align left
    latex->DrawLatex(0.06,0.96,"CMS preliminary 2012");
  }
} // cmsPrel
