...
Long64_t PTCut::LoadTree(Long64_t entry)
{
...
void PTCut::InitFuncs()
{
  ...
  fp1 = new TF1("fp1","([0]+[1]*pow(max(x,[4]),[2]))"
...
{
  inFile = new TFile("include/pfhadrons.root", "READ");
...
void PTCut::JetLoop(){
  ...
  for (unsigned int i = 0; i < sortedJets.size(); i++) {
    ...
    // Check that this is a sensible jet (pt sorting, because ???)
...
void PTCut::ParticleLoop(){
  ...
  for ( unsigned int j = 0; j != jetParts.size(); ++j ){
    int absId = abs( jetParts[j].user_index() ); 
...
void PTCut::WriteResults(){
  outFile = new TFile("ptcut.root", "RECREATE");
